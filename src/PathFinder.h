...
/**
	...
	@tparam TNode The type of nodes you are working on. It <b>must</b> derive
...
public:
    ...
    */
    ...
    /**
        ...
        @param[in] goal A reference to the goal node.
    */
    ...
    */
    ...
    /**
        ...
        @tparam TAlgorithm The algorithm to use (for example : AStar).
    ...
    */
    ...
    bool findPath(std::vector<TNode *> &solution, int hint = 0) {
        ...
        if(hint > 0) {
            path.reserve((unsigned long) hint);
        }
        ...
        if(hint > 0) {
            solution.reserve((unsigned long) hint);
        }
